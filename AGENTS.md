...
- 운영 호스트 기본은 EC2 **t4g.small**(arm64, 2GiB) + Ubuntu Server **24.04** LTS다. Dockerfile에 `linux/amd64`를 굽지 않는다. SQLite·RDS·프로필 PG 저장을 넣지 않는다. HTTP 1차 배포 명령은 `TECH_STACK.md` §5.1.2다.
