...
| 기능정의서 PDF | Ubuntu 24.04, EC2 t3a.medium | Ubuntu **26.04 LTS**, EC2 **t4g.medium 후보** | 사용자 지정 ARM64 방향. 인스턴스 적합성·비용은 EC2 실측 전 미확정 |
...
```text
[Host] AWS EC2 t4g.medium 후보 (2 vCPU, 4 GiB, arm64) + EBS gp3 30GB
...
| 항목 | 값 | 근거 |
|------|-----|------|
| 인스턴스 | **t4g.medium 후보** (2 vCPU / 4 GiB, Graviton2) | [AWS T4g 공식 사양](https://aws.amazon.com/ec2/instance-types/t4/). 버스트 인스턴스이며 6컨테이너 workload 측정 전까지 충분하다고 확정하지 않음 |
| 디스크 | gp3 30 GB | 이미지+PG+로그 |
| 리전 | ap-northeast-2 | 지연, 공공 API |
| 공개 포트 | 443 (80은 리다이렉트), SSH는 본인 IP | 3000/8000/5432/6379 비공개 |
| Worker | `CELERY_CONCURRENCY=1` | PDF 피크 메모리를 줄이는 P0 설정. 4GiB 적합성은 측정 대기 |
...
Ubuntu 26.04 ARM64 AMI는 [Canonical의 AWS 이미지 조회 절차](https://ubuntu.com/aws/docs/aws-how-to/instances/find-ubuntu-images/)에 따라 배포 시 AWS 공개 SSM 파라미터로 조회한다. 성공 여부만 기록하고 AMI 값을 문서에 고정하지 않는다.

```bash
aws ssm get-parameters --region ap-northeast-2 --names /aws/service/canonical/ubuntu/server/26.04/stable/current/arm64/hvm/ebs-gp3/ami-id
```

인스턴스 적합성은 Compose 기동 → 캐시 통계 조회 → PDF 마스킹 중 `docker stats --no-stream`과 호스트 메모리·CPU credit을 측정해 판단한다. 현재 측정 결과는 **미검증**이다. 비용도 AWS Pricing Calculator 확인 전에는 수치를 확정하지 않는다.
...
**EC2 초기 설정 (요지):** Ubuntu 26.04 · `apt update && apt upgrade` · Docker Engine + Compose v2 · ufw 22/80/443 · repo clone · 저장소 밖 환경 파일(`chmod 600`) · 위 **§5.1.1** (`up` → migrate → worker sync 1회).
...
- GitHub Actions는 Repository/Environment Secret을 사용한다. 이번 EC2 데모는 저장소 밖 환경 파일 방식이며 AWS Secrets Manager는 데모 후 전환 목표다.
