...
| G6 | 목적·항목·보유기간·거부권 고지(**`/consultations` 모달**)에 동의한 경우만 **이메일**(P0)·선택 메모를 암호화해 기한부 PG 저장 · 접수 시 운영 SMTP 본문에 신청자 이메일·선택 메모만 넣어 회신 가능(수신은 운영 주소만, `request_id` 비포함). 로그·HTTP 응답은 비평문 |
...
- OCR, HITL, TypeScript, Turborepo, RDS  
- **사용자 앱 로그인·회원가입.** 운영(F-09 다건 PDF, F-10 대시보드·상담 복호화 열람)은 **`/ops`** 이며 사용자 Header에 두지 않는다  
- JWT 백오피스. 운영 세션은 HMAC 쿠키 `ifa_ops`  
- 실시간 개인 보험료 견적, 가입 유도  
...
| 이가족 | 위 + (선택) PDF |
| 운영 설계사 | 상담 알림 수신 후 신청자 이메일로 **회신**. 열람 UI는 `/ops` |
...
6. (선택) **`/consultations`** 모달 동의 후 **이메일** 상담 신청 → 운영 알림(본문에 신청자 이메일·선택 메모)  
...
| 중개 오인 | 카피 검수 |
| 7일 일정 | 사용자 P0. 운영 `/ops`는 사용자 여정과 분리 |
