...
| G6 | 목적·항목·보유기간·거부권 고지(**`/consultations` 모달**)에 동의한 경우만 **이메일**(P0)·선택 메모를 암호화해 기한부 PG 저장 · 접수 시 운영 SMTP 본문에 신청자 이메일을 넣어 회신 가능(수신은 운영 주소만). 로그·HTTP 응답은 비평문 |
...
6. (선택) **`/consultations`** 모달 동의 후 **이메일** 상담 신청 → 운영 알림(본문에 신청자 이메일)  
